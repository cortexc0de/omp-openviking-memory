...
export class OVClient {
  ...
  async tree(uri: string, opts?: { levelLimit?: number; nodeLimit?: number }): Promise<any[] | null> {
    const q = new URLSearchParams({ uri });
    if (opts?.levelLimit != null) q.set("level_limit", String(opts.levelLimit));
    if (opts?.nodeLimit != null) q.set("node_limit", String(opts.nodeLimit));
    const res = await this.fetchJSON<any[]>(`/api/v1/fs/tree?${q.toString()}`, undefined, 10000);
    if (!res.ok || !Array.isArray(res.result)) return null;
    return res.result;
  }

  /** PUT /api/v1/fs/write — creates or replaces file content at viking:// URI */
  async writeFile(uri: string, content: string, mode?: string): Promise<boolean> {
    const body: Record<string, unknown> = { uri, content };
    if (mode) body.mode = mode;
    const res = await this.fetchJSON<any>("/api/v1/fs/write", { method: "PUT", body: JSON.stringify(body) }, 15000);
    return res.ok;
  }

  /** PATCH /api/v1/fs/edit — targeted string replacement in existing file */
  async editFile(uri: string, oldText: string, newText: string): Promise<boolean> {
    const res = await this.fetchJSON<any>("/api/v1/fs/edit", { method: "PATCH", body: JSON.stringify({ uri, old_text: oldText, new_text: newText }) }, 15000);
    return res.ok;
  }

  /** GET /api/v1/system/status — identity / health detail (wraps /health with more fields) */
  async systemStatus(): Promise<any | null> {
    const res = await this.fetchJSON<any>("/api/v1/system/status", undefined, 5000);
    return res.ok ? res.result : null;
  }

  /** Probe whether optional endpoints exist on this server (cached per instance) */
